                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        @if(user_can('edit_contact'))
                            <a href="{{ url('/admin/contacts/' . $contact->id . '/edit') }}" title="Edit contact">
                        ...
                        <div class="table-responsive">
                            <table class="table">
                                ...
                                <tr>
                                    <th> Status</th>
                                    <td><i class="btn bg-maroon">{{ $contact->getStatus->name }}</i></td>
                                </tr>
                                <tr>
                                    <th> Referral source</th>
                                    <td> {{ $contact->referral_cource }} </td>
                                </tr>
                                <tr>
                                ...
                                @if(\Auth::user()->is_admin == 1)
                                    ...
                                    <tr>
                                        <th> Modified by</th>
                                        <td>{{ isset($contact->modifiedBy->name)?$contact->modifiedBy->name:"" }}</td>
                                    </tr>
                                @endif

                                <tr>
                                    <th> Assigned to</th>
                                    <td>{{ $contact->assignedTo != null ?$contact->assignedTo->name : "not set" }}</td>
                                </tr>
                                <tr>
                                    <th> Created at</th>
                                ...
                                @if($contact->emails->count() > 0)
                                    <tr>
                                        <th>Emails</th>
                                        <td>{{ implode(", ", array_column($contact->emails->toArray(), "email")) }}</td>
                                    </tr>
                                @endif
                                @if($contact->phones->count() > 0)
                                    <tr>
                                        <th>Phones</th>
                                        <td>{{ implode(", ", array_column($contact->phones->toArray(), "phone")) }}</td>
                                    </tr>
                                @endif
                                @if($contact->documents->count() > 0)
                                    <tr>
                                        <th>Documents</th>
                                        <td>@foreach($contact->documents as $document) <a href="{{ url('uploads/documents/' . $document->file) }}">{{ $document->name }}</a> - @endforeach</td>
                                    </tr>
                                @endif
                                </tbody>
                            </table>
